...
static size_t process(cgltf_data* data, const char* input_path, const char* output_path, const char* report_path, const char* merge_metadata_path, std::vector<Mesh>& meshes, std::vector<Animation>& animations, const Settings& settings, std::string& json, std::string& bin, std::string& fallback, size_t& fallback_size, const char* meshopt_ext)
{
	if (settings.verbose)
	{
...
int gltfpack(const char* input, const char* output, const char* report, const char* merge_metadata, Settings settings)
{
	...
	json += '{';
	size_t bufferspec_pos = process(data, input, output, report, merge_metadata, meshes, animations, settings, json, bin, fallback, fallback_size, meshopt_ext);
	json += '}';
